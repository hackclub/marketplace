...
<div style="margin-left: 80px; margin-top: 20px;">
	{#if data.length > 0}
		{#each data as item}
            <div class="w-80 rounded-lg relative">
                <img src="{item.coverLink}" alt="Cover" class="h-40 bg-orange-500 rounded-lg mb-4" />
                <h1 class="text-black font-bold text-lg mt-6">{item.title}</h1>
