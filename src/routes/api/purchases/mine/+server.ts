...
export async function GET(req: Request) {
   ...
   return await fetch(`https://api.airtable.com/v0/${PRIVATE_AIRTABLE_BASE_ID}/heidi_money?filterByFormula=${encodeURIComponent(`{buyer} = "${sessionData.slackId}"`)}`, {
        headers: {
            Authorization: `Bearer ${PRIVATE_AIRTABLE_API_KEY}`
        }
    }).then(r => r.json()).then(dd => {
        console.log(dd.records[0])
        return new Response(JSON.stringify(dd.records.map(r => ({
            id: r.id,
