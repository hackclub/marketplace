            ...
            <div class="mt-6">
                ...
                <div id="attributes">
                  <!-- is there a better way to do this? yes, did it work? no. -->
                   {#if item.status === "shipped!"}
                    <span class={`my-2 inline-block rounded-full px-6 pb-2 pt-2.5 font-medium shadow-md transition duration-150 ease-in-out hover:bg-green-500 hover:shadow-lg bg-green-400 text-white`}>{item.status}</span>
                    {:else if item.status !== "shipped!" && item.status !== "draft"}
                    <span class={`my-2 inline-block rounded-full px-6 pb-2 pt-2.5 font-medium shadow-md transition duration-150 ease-in-out hover:bg-yellow-500 hover:shadow-lg bg-yellow-400 text-yellow-700`}>{item.status}</span>
                    {/if}
                    {#if item.status === "draft"}
                    <span class={`my-2 inline-block rounded-full px-6 pb-2 pt-2.5 font-medium shadow-md transition duration-150 ease-in-out hover:bg-red-500 hover:shadow-lg bg-red-400 text-white`}>{item.status}</span>
                    {/if}
                </div>
            </div>
        </div>
